...
Backend tests cover staff signature/audience/expiry/suspension/role/CSRF checks; private evidence and staff notes; price separation; short and excess receipt; immutable correction history; return recovery; partner unavailability; cancellation/pickup races; demand reuse; and idempotent retries. Native tests cover photo processing, draft recovery, lost-response replay, separate charge acknowledgment and receipt entry on Android 16. They exercise real native screens against a controlled in-process API; backend integration tests exercise actual SQLite transactions separately.

Verified code commit `3c4acd014ac40404d049c188604d6e99deb9629d` in [GitHub Actions run 35931307380](https://github.com/orcazen9-png/application-ewaste/actions/runs/35931307380): **74 backend tests passed**, JavaScript checks and build passed, native compilation passed, and **9 Android 16 tests passed with zero failures, errors or skips**. The native logistics screenshot was downloaded and visually inspected. [Local screenshot](../work/logistics-verified/results/native-logistics.png) is an ignored local artifact; the run's `account-foundation-test-results` download includes the reports and screenshots and expires on 30 September 2026. These results are not evidence of real-provider or physical-phone testing.

All five migrations were successfully applied to a fresh **local** Cloudflare D1 runtime. Browser verification used a loopback-only isolated SQLite/R2 fixture with locally signed test identities: opened the order, recorded an issue and resolved it, retaining the timeline. A final INR 1,000 logistics revision left the INR 10,000 material amount and unpaid state unchanged, reset recycler acknowledgment and retained readable actor/source/reason history. The temporary preview server and browser tab were closed after verification.
