...
Download the `.apk` directly on Android. This release uses the same signing certificate and package as native v3. The build tests an update from the actual previously distributed v3 APK on an Android 16 emulator. Physical-phone verification is separate. Do not uninstall an existing app to resolve an error without preserving its unsynced records.
...
Earlier workspace data is retained in the same package but is not automatically assigned to personal accounts. The owner must review any migration. Detailed category names and some technical validation messages remain in English.
