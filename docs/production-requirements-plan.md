...
The `codex/marketplace-orders` branch adds the native recycler portfolio, collector matching/review, shared requests/orders, atomic reservations, acknowledged price revisions and opt-in photo identification. See the [marketplace milestone](marketplace-orders.md) for its implemented scope and limits. Native v3's simulated completion writes the entire amount as paid; this behavior is not part of the new order flow. Milestone verification does not establish the entire production checklist or a completed live rollout.

The `codex/logistics-receipt` branch adds core M4: restricted staff identity checks and operations portal, partner/schedule/cost history, native handover and inspection evidence, quantity acceptance, issues and confirmed returns. See the [logistics implementation and activation boundary](logistics-receipt.md). Cloudflare Access policy and staff provisioning are still required before live activation. Complex custody reversals, write-offs, complete operations administration and the M5 invoice/payment ledgers remain planned; delivery does not mark payment.
