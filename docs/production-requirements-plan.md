...
The normalized transaction system remains planned. Native v3's simulated completion writes the entire amount as paid; this behavior must be removed from live flows, not carried into the new ledger. Account-foundation verification covers its account/draft scope, not the entire production checklist.
