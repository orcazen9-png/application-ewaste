...
Current Cloudflare records are workspace JSON plus D1 photo bytes; individual accounts, R2 and the normalized transaction system remain planned. Native v3's simulated completion writes the entire amount as paid; this behavior must be removed from live flows, not carried into the new ledger. Existing successful CI proves only its tested prototype flows, not this checklist.
