...
`wrangler.toml` deploys the Worker to workers.dev with a D1 database (`ewaste-demo`). The account has no R2, so photos are stored in D1 (`server/d1-photos.js`, below 1.4 MB each; the app sends ~1440 px JPEGs). `GEMINI_MODEL` is a plain variable in `wrangler.toml`; the key is a Worker secret:

```sh
npx wrangler d1 migrations apply ewaste-demo --remote   # once per new migration
npx wrangler secret put GEMINI_API_KEY                  # paste the key when asked
npm run deploy
```

`SPACE_CREATION=disabled` turns off `POST /api/spaces`: without Sites in front, anyone could send the sign-in header. One shared workspace was inserted directly into D1. Its join link, `https://<worker>/identify.html#join=<code>`, pairs a phone on first open; the code stays in the URL fragment (never sent to the server) and is removed from the address bar. The code and QR image are kept only in ignored `work/demo-share/`. Anyone holding the link can view and operate the whole demo workspace, so share it only with demo participants.

Free-plan CPU (10 ms/request): photo base64 uses native `Buffer` (`nodejs_compat`); the hand-written loop cost ~30 ms per 500 KB photo. Waiting for Gemini is not CPU time.
