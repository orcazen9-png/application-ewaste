...
async function matches(request,env,user){
  ...
  const facilityId=url.searchParams.get('facilityId')||'';if(facilityId)requireId(facilityId);
  const page=await rows(env.DB.prepare(requirementSelect+" WHERE (?='' OR r.facility_id=?) AND r.broad_code=? AND r.id>? AND u.status='active' AND r.state='active' AND ("+reviewed+" OR (?=1 AND EXISTS(SELECT 1 FROM demo_facility_access d WHERE d.facility_id=f.id AND d.expires_at>strftime('%Y-%m-%dT%H:%M:%fZ','now')))) AND r.valid_until>? ORDER BY r.id LIMIT 51").bind(facilityId,facilityId,s.item.broad_code||'',cursor(request),env.DEMO_MODE==='true'?1:0,now()));
...
async function requestsRoute(request,env,user,record,action){
  ...
  if(!action){
    const initiated=user.role==='recycler';let seller=user,listing=null;
    if(initiated){listing=await postedLot(env,input.lotId);if(!listing||listing.listing_version!==input.listingVersion)fail('The posted lot changed. Refresh before quoting.',409);seller={id:listing.owner_user_id,role:'collector'};}
    else requireRole(user,'collector');
    const s=await source(env,seller,input.lotId,input.itemId),r=await getRequirement(env,input.requirementId);
    if(initiated&&r?.owner_id!==user.id)fail('Choose your own buying requirement.',403);
    ...
    return commit(env,c,first,{id:record,version:1},[(g,a)=>env.DB.prepare(`INSERT INTO request_files(request_id,file_id) SELECT ?,file_id FROM lot_files WHERE lot_id=? AND ${g}`).bind(record,s.lot.id,...a),event(env,record,user.id,'submitted',initiated?'Recycler offered to buy posted material':'Collector submitted the reviewed material proposal')]);
  }
  const row=await permittedRequest(env,user,record);
  if(action==='message'){
    const message=text(input.message,2000,'message');if(!message)fail('Enter a message.');
    const count=await env.DB.prepare('SELECT count(*) AS n FROM market_events WHERE actor_id=? AND kind=? AND created_at>?').bind(user.id,'message',new Date(Date.now()-3600000).toISOString()).first();if(count.n>=120)fail('Too many messages. Try again later.',429);
    return commit(env,c,env.DB.prepare('INSERT INTO market_events(id,request_id,actor_id,kind,message,created_at) VALUES(?,?,?,?,?,?)').bind(id(),record,user.id,'message',message,time),{id:record,version:row.version});
  }
  if(row.version!==version(input))fail('Request changed. Refresh before responding.',409);
  if(!activeStates.includes(row.state))fail('This request no longer accepts responses.',409);
  if(row.expires_at<=time)fail('This offer expired. Start a new proposal.',409);
  if(action==='quote'){
    const amount=money(input.amount),message=text(input.message,2000,'quote note');
    if(input.offerVersion!==row.offer_version)fail('A newer offer is available. Refresh first.',409);
    return commit(env,c,env.DB.prepare("UPDATE supply_requests SET ask_paise=?,proposed_by=?,offer_version=offer_version+1,version=version+1,updated_at=? WHERE id=? AND version=? AND offer_version=? AND state IN ('submitted','clarification')").bind(amount,user.id,time,record,row.version,row.offer_version),{id:record,version:row.version+1},[event(env,record,user.id,'quote',JSON.stringify({amount:rupees(amount),offerVersion:row.offer_version+1,note:message}))]);
  }
  ...
  if(action==='accept'){
    if((row.proposed_by||row.collector_id)===user.id)fail('The other party must accept this offer.',403);
    if(row.offer_version>1&&input.offerVersion!==row.offer_version||input.offerVersion!==undefined&&input.offerVersion!==row.offer_version)fail('Review the latest offer before accepting.',409);
    ...
    return commit(env,c,first,{id:orderId,requestId:record,version:1},[(g,a)=>env.DB.prepare(`UPDATE order_terms SET proposed_by=?,acknowledged_by=?,reason='Material offer accepted by the other party' WHERE order_id=? AND version=1 AND ${g}`).bind(row.proposed_by||row.collector_id,user.id,orderId,...a),event(env,record,user.id,'accepted','Material offer accepted; supply and demand reserved. Recycler pays logistics separately.')]);
  }
  ...
  else if(action==='reject'&&(row.proposed_by||row.collector_id)!==user.id)state='rejected';
