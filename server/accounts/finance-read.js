...
export async function personalInsights(request,env,user,path){
  if(path==='/api/v1/earnings'&&request.method==='GET'){
    const list=await rows(env.DB.prepare(`SELECT o.id,o.created_at,o.state,${paymentColumns} FROM orders o WHERE o.collector_id=? OR o.recycler_id=? ORDER BY o.created_at DESC`).bind(user.id,user.id));
    return json({role:user.role,currency:'INR',confirmed:moneyText(list.reduce((n,o)=>n+o.confirmed_paise,0)),approved:moneyText(list.reduce((n,o)=>n+(o.invoice_paise||0),0)),outstanding:moneyText(list.reduce((n,o)=>n+Math.max(0,(o.invoice_paise||0)-o.confirmed_paise),0)),orders:list.map(o=>({id:o.id,createdAt:o.created_at,state:o.state,paymentState:paymentState(o),approved:o.invoice_paise==null?null:moneyText(o.invoice_paise),confirmed:moneyText(o.confirmed_paise)}))});
  }
  if(path==='/api/v1/notifications'&&request.method==='GET'){
    const cursor=new URL(request.url).searchParams.get('before')||now();if(!Number.isFinite(Date.parse(cursor)))fail('Invalid notification cursor.');
    const list=await rows(env.DB.prepare('SELECT * FROM notifications WHERE user_id=? AND created_at<=? ORDER BY created_at DESC,id DESC LIMIT 101').bind(user.id,cursor));
    ...
    return json({notifications:list.slice(0,100),unread:unread.total,hasMore:list.length>100});
  }
