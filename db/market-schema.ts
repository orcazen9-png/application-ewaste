...
export const supplyRequests=sqliteTable('supply_requests',{
  ...
  quantityBase:integer('quantity_base').notNull(),unit:text('unit').notNull(),mode:text('mode').notNull(),askPaise:integer('ask_paise'),proposedBy:text('proposed_by').references(()=>users.id),offerVersion:integer('offer_version').notNull().default(1),listingVersion:integer('listing_version'),snapshotJson:text('snapshot_json').notNull(),state:text('state').notNull(),version:integer('version').notNull(),expiresAt:text('expires_at').notNull(),createdAt:text('created_at').notNull(),updatedAt:text('updated_at').notNull()
...
export const reservations=sqliteTable('reservations',{
  orderId:text('order_id').primaryKey().notNull().references(()=>orders.id),requirementId:text('requirement_id').notNull().references(()=>requirements.id),lotId:text('lot_id').notNull().references(()=>lots.id),itemId:text('item_id').notNull(),quantityBase:integer('quantity_base').notNull(),demandBase:integer('demand_base'),state:text('state').notNull()
