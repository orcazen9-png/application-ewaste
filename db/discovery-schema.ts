...
export const lotListings=sqliteTable('lot_listings',{
 lotId:text('lot_id').primaryKey().notNull().references(()=>lots.id),ownerId:text('owner_id').notNull().references(()=>users.id),lotVersion:integer('lot_version').notNull(),version:integer('version').notNull(),state:text('state').notNull(),latitude:real('latitude'),longitude:real('longitude'),createdAt:text('created_at').notNull(),updatedAt:text('updated_at').notNull()
