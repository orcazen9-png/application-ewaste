...
export const lots=sqliteTable('lots',{
  id:text('id').primaryKey().notNull(),ownerUserId:text('owner_user_id').notNull().references(()=>users.id),title:text('title').notNull(),locality:text('locality').notNull(),notes:text('notes').notNull(),status:text('status').notNull().default('draft'),version:integer('version').notNull(),lastCommand:text('last_command').notNull(),createdAt:text('created_at').notNull(),updatedAt:text('updated_at').notNull()
...
export const lotItems=sqliteTable('lot_items',{
  lotId:text('lot_id').notNull().references(()=>lots.id),id:text('id').notNull(),taxonomyVersion:text('taxonomy_version').notNull().references(()=>taxonomyVersions.version),broadCode:text('broad_code'),detailedCode:text('detailed_code'),description:text('description').notNull(),condition:text('condition').notNull(),unit:text('unit').notNull(),quantityBase:integer('quantity_base'),reviewState:text('review_state').notNull()
