...
for(const line of (await readFile('native-android/ui-translations.tsv','utf8')).split(/\r?\n/)){if(!line)continue;const values=line.split('\t');if(values.length!==3||values.some(v=>!v))throw new Error('Invalid translation row: '+values[0]);output[values[0]]=values.slice(1);}
