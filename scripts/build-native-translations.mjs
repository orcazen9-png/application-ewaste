...
for(const source of ['ui-translations','error-translations','redesign-translations','entry-translations','facility-translations','discovery-translations'])for(const line of (await readFile('native-android/'+source+'.tsv','utf8')).split(/\r?\n/)){if(!line)continue;const values=line.split('\t').map(v=>v.replaceAll('\\n','\n').replace(/\\s$/,' '));if(values.length!==3||values.some(v=>!v))throw new Error('Invalid translation row: '+values[0]);output[values[0]]=values.slice(1);}
