...
export async function localBindings(directory){
  ...
  const prepare=sql=>{let args=[];return {bind(...values){args=values;return this;},async first(){return database.prepare(sql).get(...args)||null;},run(){const result=database.prepare(sql).run(...args);return {meta:{changes:Number(result.changes)}};}};};
  ...
  return {DB,BUCKET,LOCAL_DEMO:true,close:()=>database.close()};
}
