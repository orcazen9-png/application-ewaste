 ...
 void form(JSONObject d)throws Exception{
  ...
  LinearLayout card=ui.card(a.page,AccountDesign.WHITE);ui.inside(card,()->{
   if(step==0){field(d,"name","Company or facility name",120);field(d,"address","Facility address",600);field(d,"locality","City or area",120);field(d,"contact","Business contact",200);field(d,"phone","Business phone",30);field(d,"email","Business email",200);
    ui.action(card,a.t("Use facility location"),"facility-location",()->a.nearby.get(point->{m.put(p,"location",point);remember(d);a.render();}),0);ui.action(card,a.t("Choose facility city or area"),"facility-area",()->a.nearby.chooseArea(p.optString("address")+" "+p.optString("locality"),point->{m.put(p,"location",point);remember(d);a.render();}),0);if(p.optJSONObject("location")!=null){ui.note(card,a.t("Approximate location added"));ui.action(card,a.t("Remove location"),"facility-clear-location",()->{p.remove("location");remember(d);a.render();},0);}}
