...
final class FinanceScreens {
    ...
    void render(JSONObject d)throws Exception{
        ...
        JSONArray accounts=d.optJSONArray("accounts");if(accounts!=null)for(int i=0;i<accounts.length();i++){JSONObject s=accounts.getJSONObject(i);a.label(s.optString("account").equals("material")?a.t("Material account"):a.t("Logistics account"),20);a.label(a.t("Approved ₹")+s.optString("approved")+"\nConfirmed ₹"+s.optString("confirmed")+" · Pending ₹"+s.optString("pending")+"\nOutstanding ₹"+s.optString("outstanding")+" · Overpaid ₹"+s.optString("overpaid"),16);if(s.optBoolean("basisChanged"))a.label(a.t("Terms changed. Review a revised invoice before further settlement."),15);}
    ...
    void earnings(JSONObject d)throws Exception{a.label(d.optString("role").equals("collector")?a.t("Your earnings"):a.t("Material payments"),23);a.label(a.t("Confirmed ₹")+d.optString("confirmed")+"\nApproved invoices ₹"+d.optString("approved")+"\nOutstanding ₹"+d.optString("outstanding"),19);a.label(a.t("Totals use collector-confirmed material payments. Logistics is accounted separately."),15);JSONArray orders=d.optJSONArray("orders");if(orders!=null)for(int i=0;i<orders.length();i++){JSONObject o=orders.getJSONObject(i);a.label(o.optString("paymentState")+" · ₹"+o.optString("confirmed"),17);a.button(a.t("Open financial record"),"earnings-order-"+i,()->open(o.optString("id")));}}
    void inbox(JSONObject d)throws Exception{a.label(a.t("Notifications"),23);a.label(d.optInt("unread")+" unread",16);JSONArray list=d.optJSONArray("notifications");if(list==null)return;for(int i=0;i<list.length();i++){JSONObject n=list.getJSONObject(i);a.label(a.t(n.optString("title"))+(n.isNull("read_at")?" · New":""),18);a.label(a.t(n.optString("body"))+"\n"+a.localDate(n.optString("created_at")),15);a.button(a.t("View update"),"notification-"+i,()->{String token=a.token();a.task(()->a.api.request("POST","/notifications/"+n.optString("id")+"/read",token,new JSONObject()),result->{if(n.optString("kind").startsWith("finance."))open(n.optString("resource_id"));else if(n.optString("kind").startsWith("logistics."))m.logistics.open(n.optString("resource_id"));else m.load("request","/requests/"+n.optString("resource_id"));});});}if(d.optBoolean("hasMore"))a.label(a.t("Showing your latest 100 notifications."),14);}
}
