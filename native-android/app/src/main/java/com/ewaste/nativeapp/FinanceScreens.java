...
final class FinanceScreens {
    ...
    String amount(JSONObject d,String key){if(!d.has(key))return "—";try{java.text.NumberFormat format=java.text.NumberFormat.getCurrencyInstance(java.util.Locale.forLanguageTag(a.language()+"-IN"));format.setCurrency(java.util.Currency.getInstance("INR"));return format.format(new java.math.BigDecimal(d.getString(key)));}catch(Exception e){return "—";}}
    void earnings(JSONObject d)throws Exception{
        AccountDesign ui=a.ui;boolean collector=a.session.optJSONObject("user").optString("role").equals("collector");
        ui.heading(a.t(collector?"Your earnings":"Material payments"),a.t("A clear view of every payment."));
        LinearLayout hero=ui.card(a.page,AccountDesign.GREEN);ui.pill(hero,a.t(collector?"Confirmed receipts":"Confirmed payments"),AccountDesign.LIME);ui.space(hero,20);
        TextView total=ui.text(amount(d,"confirmed"),38,AccountDesign.WHITE,true);total.setMaxLines(1);total.setAutoSizeTextTypeUniformWithConfiguration(22,38,1,android.util.TypedValue.COMPLEX_UNIT_SP);hero.addView(total,new LinearLayout.LayoutParams(-1,a.dp(56)));ui.space(hero,8);hero.addView(ui.text(a.t(collector?"Money you have confirmed receiving":"Material payments confirmed by collectors"),13,0xffd4e5d9,false));
        LinearLayout pending=ui.card(a.page,AccountDesign.AMBER);ui.keyValue(pending,a.t("Awaiting confirmation"),amount(d,"pending"));ui.note(pending,a.t("Recorded transfers that still need receipt confirmation."));
        LinearLayout summary=ui.card(a.page,AccountDesign.WHITE);summary.addView(ui.text(a.t("Payment summary"),18,AccountDesign.INK,true));ui.space(summary,8);ui.keyValue(summary,a.t("Accepted quotes"),amount(d,"expected"));ui.divider(summary);ui.keyValue(summary,a.t("Approved invoices"),amount(d,"approved"));ui.divider(summary);ui.keyValue(summary,a.t("Outstanding dues"),amount(d,"outstanding"));
        ui.section(a.t("Payment activity"));JSONArray orders=d.optJSONArray("orders");
        if(orders!=null&&orders.length()==0){LinearLayout empty=ui.card(a.page,AccountDesign.WHITE);empty.addView(ui.iconView("wallet",AccountDesign.GREEN,30),new LinearLayout.LayoutParams(a.dp(30),a.dp(30)));ui.space(empty,14);empty.addView(ui.text(a.t("No payments yet"),19,AccountDesign.INK,true));ui.note(empty,a.t(collector?"Your receipts will appear here as your lots become orders.":"Your material payment history will appear here."));if(collector)ui.action(empty,a.t("Create a lot"),"earnings-create-lot",a::createDraft,0);}
        if(orders!=null)for(int i=0;i<orders.length();i++){JSONObject o=orders.getJSONObject(i);ui.link(a.page,amount(o,"confirmed"),a.t(o.optString("paymentState"))+" · "+a.localDate(o.optString("createdAt")),"earnings-order-"+i,"wallet",()->open(o.optString("id")));}
        ui.note(a.page,a.t("Only confirmed receipts count as earnings. Transport is accounted separately. Quotes can change."));
    }
    void inbox(JSONObject d)throws Exception{a.label(a.t("Notifications"),23);a.label(d.optInt("unread")+a.t(" unread"),16);JSONArray list=d.optJSONArray("notifications");if(list==null)return;for(int i=0;i<list.length();i++){JSONObject n=list.getJSONObject(i);a.label(a.t(n.optString("title"))+(n.isNull("read_at")?a.t(" · New"):""),18);a.label(a.t(n.optString("kind").startsWith("finance.")?a.t("Open the order to review this update."):n.optString("body"))+"\n"+a.localDate(n.optString("created_at")),15);a.button(a.t("View update"),"notification-"+i,()->{String token=a.token();a.task(()->a.api.request("POST","/notifications/"+n.optString("id")+"/read",token,new JSONObject()),result->{if(n.optString("kind").startsWith("chat."))m.load("chat","/conversations/"+n.optString("resource_id"));else if(n.optString("kind").startsWith("finance."))open(n.optString("resource_id"));else if(n.optString("kind").startsWith("facility."))m.facility.open();else if(n.optString("kind").startsWith("logistics."))m.logistics.open(n.optString("resource_id"));else m.load("request","/requests/"+n.optString("resource_id"));});});}if(!d.isNull("nextCursor")&&!d.optString("nextCursor").isEmpty())a.button(a.t("Older notifications"),"inbox-older",()->{try{m.load("inbox","/notifications?before="+java.net.URLEncoder.encode(d.optString("nextCursor"),"UTF-8"));}catch(Exception e){a.showError(e);}});
