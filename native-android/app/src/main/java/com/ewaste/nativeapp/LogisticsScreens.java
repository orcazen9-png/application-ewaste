...
final class LogisticsScreens {
    ...
    void render(JSONObject d)throws Exception{
        ...
        a.label(a.t("Delivery: ")+a.t(j.optString("state")),20);a.label(a.t("Material: ₹")+o.optString("materialAmount")+a.t(" · Payment: ")+a.t(o.optString("paymentState")),16);
        a.label(a.t("Requested: ")+j.optString("requested")+" "+unit+a.t("\nAccepted receipt: ")+(j.isNull("acceptedQuantity")?a.t("Awaiting review"):j.optString("acceptedQuantity")+" "+unit)+"\nReturned: "+j.optString("returnedQuantity")+" "+unit,16);
        ...
        JSONArray cases=d.optJSONArray("cases");if(cases!=null&&cases.length()>0){a.label(a.t("Issues & review"),20);for(int i=0;i<cases.length();i++){JSONObject c=cases.getJSONObject(i);a.label(a.t(c.optString("state"))+" · "+c.optString("reason")+(c.isNull("resolution")?"":"\n"+c.optString("resolution")),15);}}
        a.label(a.t("Evidence & activity"),20);JSONArray records=d.optJSONArray("records");if(records!=null)for(int i=0;i<records.length();i++){
            JSONObject r=records.getJSONObject(i),e=r.getJSONObject("data");a.label(a.t(r.optString("kind"))+" · "+a.localDate(r.optString("createdAt")),15);
    ...
    void evidence(JSONObject d)throws Exception{
        a.label(a.t(d.optString("kind")).equals("pickup")?a.t("Record handover"):d.optString("kind").equals("receipt")?a.t("Record inspection"):a.t("Confirm physical return"),23);
        ...
        a.label(a.t("Evidence time: ")+a.localDate(d.getString("capturedAt")),15);
        ...
        JSONArray files=d.getJSONArray("fileIds");a.label(files.length()+a.t(" evidence photo(s)"),16);
