...
public class NativeDesignTest {
    ...
    void capture(String name)throws Exception{InstrumentationRegistry.getInstrumentation().waitForIdleSync();String prefix=InstrumentationRegistry.getArguments().getString("screenshotPrefix","ui");ParcelFileDescriptor fd=InstrumentationRegistry.getInstrumentation().getUiAutomation().executeShellCommand("screencap -p /sdcard/"+prefix+"-"+name+".png");try(InputStream in=new ParcelFileDescriptor.AutoCloseInputStream(fd)){byte[] b=new byte[4096];while(in.read(b)!=-1){}}}
    @Test public void photoFirstFlowAndPersistentNavigationRenderWithoutClippedActions()throws Exception{
        ...
        try(ActivityScenario<AccountActivity> s=ActivityScenario.launch(AccountActivity.class)){
            s.onActivity(a->{visible(a,"entry-login");visible(a,"entry-signup");});capture("welcome-saved");s.onActivity(a->a.root.findViewWithTag("entry-continue").performClick());
