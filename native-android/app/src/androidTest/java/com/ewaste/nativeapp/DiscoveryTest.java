...
public class DiscoveryTest {
 ...
 @Test public void directoryAndPostedLotsRenderNativeControls()throws Exception{
  ...
  try(ActivityScenario<AccountActivity> s=ActivityScenario.launch(AccountActivity.class)){s.onActivity(a->a.root.findViewWithTag("entry-continue").performClick());idle(s);s.onActivity(a->a.root.findViewWithTag("home-discover").performClick());idle(s);s.onActivity(a->{assertEquals("directory",a.market.view());assertNotNull(a.root.findViewWithTag("discovery-open-0"));assertNotNull(a.root.findViewWithTag("discovery-gps"));});capture("directory");
   ...
   s.onActivity(a->{try{a.session.getJSONObject("user").put("role","collector");JSONObject item=new JSONObject().put("id","price-line").put("name","Laptop").put("broadCode","B01").put("unit","piece").put("available","2").put("askingRate",JSONObject.NULL);JSONObject lot=new JSONObject().put("id",UUID.randomUUID().toString()).put("title","Office laptops").put("locality","Virar").put("items",new JSONArray().put(item));a.market.show("listing",new JSONObject().put("lot",lot).put("listing",new JSONObject().put("version",0).put("current",true)));android.widget.EditText price=a.root.findViewWithTag("listing-price-0");assertNotNull(price);price.setText("2500.50");assertEquals("2500.50",item.optString("askingRate"));}catch(Exception e){throw new AssertionError(e);}});idle(s);capture("asking-price");
   s.recreate();idle(s);s.onActivity(a->{assertEquals("2500.50",a.market.data().optJSONObject("lot").optJSONArray("items").optJSONObject(0).optString("askingRate"));try{a.session.getJSONObject("user").put("role","recycler");a.market.data().getJSONObject("lot").put("distanceKm",53).put("aggregator","Virar collector");a.render();assertNull(a.root.findViewWithTag("listing-price-0"));assertNotNull(a.root.findViewWithTag("listing-directions"));}catch(Exception e){throw new AssertionError(e);}});
